...
def segmentize(num):
  return (
    digits[num // 10 % 10] 
    | digits[num // 100 % 10] << 8
    | second_digits[num // 1000 % 10] << 16 
    | digits[0] << 24 #second_digits[num // 1000 % 10] << 24 
  )
# ---End of SevSeg---
...
def rs_interrupt(pin): # Handles interrupts generated by Reed switches
  ...
  if game_state == RUNNING and control_sig == 0:
...
while True:   # Will loop forever when the Pi is powered
  if game_state == IDLE:
    ...
    btn_pressed = 0
    btn_LED.value(1)
    sleep(0.5) #non_blocking_sleep(500) might be causing issues with restarting the game, look over again
    reset()

  else:
    # Error logic goes here, being here indicates failure in the game's state attribution, restart recommended
    write_colour_all(RED)
    sleep(0.5)
    write_colour_all(BLACK)
    sleep(0.5)
...
while True:
  # Critical error loop, can only be reached if the main loop fails, restart necessary
  write_colour_all(YELLOW)
  sleep(0.5)
  write_colour_all(RED)
  sleep(0.5)
